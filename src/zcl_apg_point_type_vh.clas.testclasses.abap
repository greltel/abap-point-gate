...
CLASS ltc_point_type_vh DEFINITION
  FINAL FOR TESTING
  RISK LEVEL HARMLESS
  DURATION SHORT.
  ...
  PRIVATE SECTION.
    ...
    DATA cut    TYPE REF TO zcl_apg_point_type_vh.
...
CLASS zcl_apg_point_type_vh DEFINITION LOCAL FRIENDS ltc_point_type_vh.

CLASS ltc_point_type_vh IMPLEMENTATION.

  METHOD given_values.
    double = NEW ltd_domain_values( values ).
    cut    = NEW zcl_apg_point_type_vh( double ).
  ENDMETHOD.

  METHOD given_values_then_mapped.
    " ARRANGE
    given_values( VALUE #( ( code = '1' description = 'First' )
    ...
    cl_abap_unit_assert=>assert_equals( act = values[ 1 ]-pointtype
                                        exp = '1'
  ...
  METHOD given_values_then_ordered.
    ...
    given_values( VALUE #( ( code = '2' description = 'Second' )
  ...
  METHOD when_read_then_right_elem.
    ...
    cl_abap_unit_assert=>assert_equals( act = double->requested_element
                                        exp = `ZAPG_POINT_TYPE`
                                        msg = `The query must read the fixed values of ZAPG_POINT_TYPE` ).
  ENDMETHOD.
